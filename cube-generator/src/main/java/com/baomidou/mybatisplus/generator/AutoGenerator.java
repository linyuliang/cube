...
public class AutoGenerator {
  ...
  private Map<String, VelocityContext> analyzeData(ConfigBuilder config) {
    ...
    for (TableInfo tableInfo : tableList) {
      ...
      ctx.put("StringUtils", new org.apache.commons.lang.StringUtils());
  ...
  private void batchOutput(String entityName, VelocityContext context) {
    try {
      ...
      pathInfo.entrySet().stream().forEach(entry -> {
        String file = String.format(
            (entry.getValue() + File.separator + tableInfo.getFileNameMap().get(entry.getKey())
                + config.getPackageConfig().getPackageInfoMap().get(entry.getKey())
                .getFileSuffix()), entityName);
        // 根据override标识来判断是否需要创建文件
        if (isCreate(file)) {
          try {
            vmToFile(context,
                config.getPackageConfig().getPackageInfoMap().get(entry.getKey()).getTemplatePath(),
                file);
          } catch (IOException e) {
