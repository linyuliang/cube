...
import com.baomidou.mybatisplus.generator.config.po.GeneratedKeyInfo;
import com.baomidou.mybatisplus.generator.config.po.GeneratedTableInfo;
...
public class ConfigBuilder {
  ...
  private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
    ...
    config.getPackageInfoMap().entrySet().stream().forEach(entry -> {
      packageInfo.put(entry.getKey(), joinPackage("", entry.getValue().getPackageName()));
    });

    // 生成路径信息
    pathInfo = new HashMap<>();
    config.getPackageInfoMap().entrySet().stream().forEach(entry -> {
      pathInfo.put(entry.getKey(),
          joinPath(entry.getValue().getOutputDir(), packageInfo.get(entry.getKey())));
    });
  }
  ...
  private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy,
    ...
    for (TableInfo tableInfo : tableList) {
      ...
      packageConfig.getPackageInfoMap().entrySet().stream().forEach(entry -> {
        tableInfo.getFileNameMap().put(entry.getKey(),
            String.format(entry.getValue().getFileName(), tableInfo.getEntityName()));
      });
    ...
    try {
      ...
      while (results.next()) {
        ...
        if (StringUtils.isNotEmpty(tableName)) {
          ...
          if (isInclude) {
            for (GeneratedTableInfo includeTab : config.getInclude()) {
              if (includeTab.getTableName().equalsIgnoreCase(tableName)) {
                tableInfo.setGeneratedKeyInfoMap(includeTab.getGeneratedKeyInfoMap());
                includeTableList.add(tableInfo);
              } else {
                notExistTables.add(includeTab.getTableName());
              }
            }
          } else if (isExclude) {
    ...
    try {
      ...
      if (QuerySQL.POSTGRE_SQL == querySQL) {
        tableFieldsSql = String
            .format(tableFieldsSql, dataSourceConfig.getSchemaname(), tableInfo.getName());
      } else {
      ...
      while (results.next()) {
        ...
        if (isId) {
          ...
          if (generatedKeyInfoMap.containsKey(field.getName())) {
            field.setGeneratedKeyInfo(generatedKeyInfoMap.get(field.getName()));
          }
        }
        // 处理其它信息
        ...
        if (field.isKeyIdentityFlag()) {//如果是自增长的ID主键，则记录属性名称，mapper使用
          tableInfo.setKeyIdentityPropertyName(field.getPropertyName());
        }
